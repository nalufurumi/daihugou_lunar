...
export function playGroup(state, playerIndex, cards, options = {}) {
  ...
  // 8切り判定: 8を含むペア系の組を出すと場が流れ、出した本人の手番が続く
  // (階段に8が含まれる場合は今のところ対象外。後日まとめて調整予定)
  const isEightCut = playType === 'group' && cards.some((c) => c.rank === 8);

  // 数字による特殊効果 (ペア系のみ対象。8切りとは別の分岐として処理する)
  let skipCount = 0;
  let reverseTriggered = false;

  if (!bypassNormalCheck && playType === 'group' && representativeRank !== null) {
    if (representativeRank === 5) {
      skipCount = cards.length;
    }
    if (representativeRank === 9 && cards.length % 2 === 1) {
      reverseTriggered = true;
    }
    if (representativeRank === 7 && options.giveCards) {
      giveCardsToNextPlayer(state, playerIndex, options.giveCards);
    }
    if (representativeRank === 10 && options.discardCards) {
      discardOwnCards(state, playerIndex, options.discardCards);
    }
    if (representativeRank === 12 && options.declareRanks) {
  ...
  if (isUniversalPlay || isEightCut) {
    ...
    if (representativeRank === 11 && !isUniversalPlay) {
...
function giveCardsToNextPlayer(state, playerIndex, chosenCards) {
  ...
  for (const card of chosenCards) {
    ...
    state.hands[nextIndex].push(moved);
  }
}
